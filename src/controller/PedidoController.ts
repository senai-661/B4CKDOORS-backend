...
class PedidoController {
    ...
    static async novo(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!dados.idUsuario || dados.total === undefined) {
                res.status(400).json({ mensagem: "Campos obrigatórios: idUsuario, total." });
                return;
            }
    ...
    static async atualizar(req: Request, res: Response): Promise<void> {
        try {
            const { idPedido } = req.params;
            const dados: PedidoDTO = req.body;
            if (dados.total === undefined || !dados.status) {
                res.status(400).json({ mensagem: "Campos obrigatórios: total, status." });
                return;
            }
